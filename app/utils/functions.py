...
def plot_analisis_2():
    ...
    ax.set_title(
        'Análisis #2: Flujo de Nuevas Admisiones por Día', fontsize=16)
    ax.set_xlabel('Fecha')
...
def plot_analisis_3():
    ...
    ax.set_title(
        'Carga Promedio de Pacientes Diarios por Nivel de Atención', fontsize=16)
...
def plot_analisis_5():
    try:
        ...
        ax1.text(0.5, 0.5, 'Error al cargar datos (Día)',
                 ha='center', va='center', color='red')
        ax2.text(0.5, 0.5, 'Error al cargar datos (Mes)',
                 ha='center', va='center', color='red')
        ax1.set_axis_off()
        ax2.set_axis_off()
        return fig
    ...
    dias_ordenados = ['Monday', 'Tuesday', 'Wednesday',
    ...
    # Rotamos un poco para que no se solapen
...
def plot_analisis_6_contexto():
    ...
    sns.boxplot(
        ...
        y='servicio_troncal',
        palette='plasma',
        hue='servicio_troncal',
        ax=ax,
        fliersize=2
    )

    ax.set_title(
        'Distribución de la Duración de Estancia por Servicio (Contexto)', fontsize=16)
...
def plot_analisis_6_outliers():
    try:
        ...
        for ax in axes.flatten():
            ax.text(0.5, 0.5, 'Error al cargar datos',
                    ha='center', color='red')
            ax.set_axis_off()
        return fig
...
def plot_analisis_7():
    ...
    unique_entidades = df_mapa_filtrado.drop_duplicates(
        subset=['nombre_entidad'])
    ...
    ax.set_title(
        'Distribución Geográfica de la Demanda Hospitalaria por Entidad', fontsize=18)
    ax.axis('off')
